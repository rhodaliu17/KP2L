...
def test_LA_Pancreas(net, val_loader, args, maxdice=0, print_result=False):
    ...
    if 'LA' in args.data_dir:
        avg_metrics, std_metrics = test_calculate_metric_LA_AMC(net, val_loader.dataset, print_result=print_result, test_save_path=args.save_path)
    elif 'BraTS' in args.data_dir:
        avg_metrics, std_metrics = test_calculate_metric_BraTS_AMC(net, val_loader.dataset, print_result=print_result,test_save_path=args.save_path)
    else:
        avg_metrics, std_metrics = test_calculate_metric_Pancreas_AMC(net, val_loader.dataset, print_result=print_result, test_save_path=args.save_path)
    time_end = time.time()

...
class LAHeart(Dataset):
    ...
    def __init__(self, data_dir, list_dir, split, aug_times=1):
        ...
        if split == 'lab':
            ...
            data_path = os.path.join(list_dir,'all.txt')
            self.transform = test_transform
        else:
    ...
    def __getitem__(self, idx):
        ...
        samples = image, label, image_norm
        if self.transform:
            tr_samples = self.transform(samples)
        image_, label_, image_norm_= tr_samples
        return {'image':image_.float(), 'label':label_.long(), 'name': image_path, 'image_norm': image_norm_.float()}

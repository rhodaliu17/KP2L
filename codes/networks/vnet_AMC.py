...
class VNet_AMC(nn.Module):
    ...
    def __init__(self, n_channels=1, n_classes=2, n_filters=16, normalization='instancenorm', has_dropout=False, n_branches = 3):
        ...
        # self.block_seven_six = ConvBlock(3, n_filters * 4, n_filters * 8, normalization=normalization)

        self.block_eight = ConvBlock(2, n_filters * 2, n_filters * 2, normalization=normalization)
        self.block_eight_up = UpsamplingDeconvBlock(n_filters * 2, n_filters, normalization=normalization)
        # self.block_eight_six = ConvBlock(3, n_filters * 2, n_filters * 8, normalization=normalization)
        if has_dropout:
    ...
    def decoder(self, features):
        ...
        # x7_tmp = self.block_seven_six(x7)

        x8 = self.block_eight(x7_up)
        x8_up = self.block_eight_up(x8)
        x8_up = x8_up + x1

        # x8_tmp = self.block_eight_six(x8)
        out = []
        self.al_input = x8_up
